...
def show_movie_details(movie):
    """Render a polished, responsive MovieMind details view."""
    ...
    title = details.get("title") or movie.get(
        "title",
        "Unknown movie",
    )
    ...
    safe_tagline = html.escape(str(tagline))
    safe_genres = html.escape(str(genres))
    safe_year = html.escape(str(year))

    st.markdown(
        '<div class="details-shell">'
        '<div class="details-kicker">MOVIEMIND MOVIE PROFILE</div>'
        '</div>',
    ...
    with detail_left:
        ...
        if not poster_url:
            local_poster = get_poster_path(tmdb_id)
            if local_poster:
                st.image(
                    local_poster,
                    width="stretch",
                ...
                st.markdown(
                    '<div class="poster-fallback details-poster-fallback">'
                    'Poster unavailable'
                    '</div>',
                    unsafe_allow_html=True,
                )
    ...
    with detail_right:
        st.markdown(
            f'<div class="details-title">{safe_title}</div>',
            unsafe_allow_html=True,
        )

        meta_parts = []
        if safe_year:
            meta_parts.append(safe_year)
        if safe_genres:
            meta_parts.append(safe_genres)
        if runtime:
            meta_parts.append(f"{int(runtime)} min")
        ...
        if safe_tagline:
            st.markdown(
                f'<div class="details-tagline">{safe_tagline}</div>',
                unsafe_allow_html=True,
            )

        st.markdown(
            '<div class="details-score-grid">'
            f'<div class="details-score-card primary">'
            '<div class="details-score-label">MOVIEMIND MATCH</div>'
            f'<div class="details-score-value">{match_score}%</div>'
            '</div>'
            f'<div class="details-score-card">'
            '<div class="details-score-label">MODEL ESTIMATE</div>'
            f'<div class="details-score-value">{model_rating:.2f} / 5</div>'
            '</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            f'<strong>{rating_text}</strong>'
            '</div>'
            '<div>'
            '<span>RUNTIME</span>'
            f'<strong>{runtime_text}</strong>'
            '</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="details-section-title">Why MovieMind recommended it</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        st.markdown(
            '<div class="details-section-title">Overview</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<div class="details-overview">{safe_overview}</div>',
            unsafe_allow_html=True,
        )

        if homepage:
            st.link_button(
                "Open official movie page",
                homepage,
    ...
    st.markdown(
        ...
        'Movie metadata and ratings are supplied by TMDB. '
        'MovieMind recommendation scores are generated locally.'
        '</div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    .details-score-card.primary .details-score-value {
        color: #155eef;
    }
        ...
        for start in range(
            ...
            for position, (
                ...
                with column:
                    ...
                    if st.button(
                        "View details",
                        key=f"details_{int(movie["movieId"])}",
                        use_container_width=True,
                    ):
